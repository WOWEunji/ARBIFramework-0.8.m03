...
import kr.ac.uos.ai.arbi.monitor.control.JMSController;
...
public class ArbiFrameworkMonitor {
	...
	private boolean initMainFrame() {
		...
		if(!(initJMSController(brokerURL, monitorID, interactionManagerURL)))
			return showWarning("");
...
//		this.ltmLineColorModel.addRow(new SequenceColorBean(LTMMessageAction.RequestStream, Color.DARK_GRAY, Color.BLACK));
	...
	private boolean initJMSController(String brokerURL, String monitorID, String interactionManagerURL) {
		
		controller = new JMSController();
		if(!(controller.connect(brokerURL, monitorID, interactionManagerURL)))
			return false;
		
		controller.send(initMonitorCommand(monitorID));
		
		MessageDispatcher dispatcher = MDFactory.getMessageProcessor();
		...
		controller.setMessageListener(dispatcher);
	...
	private String initMonitorCommand(String monitorID) {
