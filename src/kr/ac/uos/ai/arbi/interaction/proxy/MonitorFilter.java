...
public class MonitorFilter {
	...
	private void setSystemLogFilter(JSONObject filterObject) {
		...
		systemLogFilter.put(new SystemLog(filterActor, filterType, filterAction), filterFlag);
	}
	...
	private boolean getSystemLogFilter(JSONObject message) {
		...
		String messageType = message.get("Type").toString().replaceAll("\"", "");
		String messageAction = message.get("Action").toString().replaceAll("\"", "");

		for (SystemLog systemLog : systemLogFilter.keySet()) {

			System.out.println("[Filter Set] \t" +" < Action > " +systemLog.getAction() + "< Actor > "+systemLog.getActor() +"< Type > " + systemLog.getType());
			
			if (systemLog.getActor().toLowerCase().equals(messageActor.toLowerCase())
					&& systemLog.getType().toLowerCase().equals(messageType.toLowerCase())
	...
	private boolean getMessageLogFilterFilter(JSONObject message) {
		...
		for (MessageLog messageLog : messageLogFilter.keySet()) {
			if (messageLog.getType().toLowerCase().equals(messageType.toLowerCase())
					&& messageLog.getAction().toLowerCase().equals(messageAction.toLowerCase())) {
	...
	public JSONArray getFilterToJSON() {
		...
		for (SystemLog systemLog : systemLogFilter.keySet()) {
			...
			filterObject.put("Type", systemLog.getType());
		...
		for (MessageLog messageLog : messageLogFilter.keySet()) {
			...
			filterObject.put("Type", messageLog.getType());
			filterObject.put("Flag", "false");
			filter.add(filterObject);
		}
